...
namespace EMedia\Oxygen\Helpers;
...
class ParseInputStream
{
    ...
    private function parameter($string)
    {
        ...
        if ( preg_match('/name=\"([^\"]*)\"[\n|\r]+([^\n\r].*)?\r$/s', $string, $match) ) {
	        if (preg_match('/^(.*)\[\]$/i', $match[1], $tmp)) {
	            $data[$tmp[1]][] = (isset($match[2]) ? $match[2] : '');
	        } else {
	            $data[$match[1]] = (isset($match[2]) ? $match[2] : '');
	        }
		}
	...
	function parse_parameter( &$params, $parameter, $value ) {
		if ( strpos($parameter, '[') !== FALSE ) {
			$matches = array();
			if (preg_match('/^([^[]*)\[([^]]*)\](.*)$/', $parameter, $match) && count($match) >= 4) {
				$name = $match[1];
				$key = $match[2];
				$rem = $match[3];
